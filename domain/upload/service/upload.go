...
import (
	...
	"download/domain/upload"
	"download/domain/upload/s3"
	"fmt"
	"io"
	"log"
)
...
type Upload struct {
	...
	uploadServices map[UploadDestination]upload.Uploader
}
...
func NewUpload(fileName string, body io.Reader, userFlag *model.UserFlag, cfgs ...Configuration) *Upload {
	u := &Upload{
		...
		body:           body,
		userFlag:       userFlag,
		uploadServices: make(map[UploadDestination]upload.Uploader),
	}
...
func WithS3Uploader() Configuration {
	return func(os *Upload) error {
		os.uploadServices[DestinationS3] = s3.NewS3Upload()
		return nil
	}
}
